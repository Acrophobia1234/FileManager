...
BOOL _CVolume::InitVolumeUSN()
{
	...
	//FSCTL_QUERY_USN_JOURNAL:Queries for information on the current update sequence number (USN)
	...
	//如果当前卷的状态未被激活，则函数会返回0，即失败，此时用FSCTL_CREATE_USN_JOURNAL，创建日志信息
	CREATE_USN_JOURNAL_DATA v1;
	v1.MaximumSize = 0x800000;//8MB
	v1.AllocationDelta = 0x100000;
	if (!IsOk &&GetLastError() == ERROR_JOURNAL_NOT_ACTIVE) {
		//激活USN
		IsOk = DeviceIoControl(DiskVolumeHandle, FSCTL_CREATE_USN_JOURNAL,&v1,sizeof(v1),  USNJournalData,
			sizeof(USN_JOURNAL_DATA), &ReturnLength, NULL);
	}
	if (IsOk)
	{
		...
		//FSCTL_ENUM_USN_DATA:Enumerates the update sequence number (USN) data between 
		//two specified boundaries to obtain master file table (MFT) records.
		//枚举USN数据
	...
	else
	{
		...
		case ERROR_JOURNAL_NOT_ACTIVE:
			//An attempt is made to write a USN record 
			//or to read the change journal while the journal is inactive.
			//日志没有被激活
		{
		...
		case ERROR_INVALID_PARAMETER:
			...
			//For example, DeviceIoControl returns this error code 
			//if the handle supplied is not a volume handle.
		{
			
			break;
		}
		case ERROR_JOURNAL_DELETE_IN_PROGRESS:
			//An attempt is made to read from, create, delete, or modify the journal 
			//while a journal deletion is in process, 
			//or an attempt is made to write a USN record while a journal deletion is in process.
